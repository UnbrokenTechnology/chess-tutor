...
def emit_rust(knots, shape, tbl):
    ...
    out = [
        "// GENERATED by calibration/gen_lookup.py from "
        "runs/grid/grid_results.csv.",
        "// Do not edit by hand; re-run gen_lookup.py to regenerate. Baked 5-D",
        "// Elo lookup over (depth, qsearch, perception, avg_move_rank, endgame),",
        "// the measured grid the calibration forward model interpolates.",
        "// `include!`d into calibration.rs (hence plain `//`, no inner docs).",
        "",
    ]
    for nm, ks in zip(names, knots):
        # repr(float(..)) always yields a decimal point ("1.0", "3.5", "10.0")
        # so the knots are valid Rust f32 literals (bare "1" would not be).
        arr = ", ".join(repr(float(k)) for k in ks)
