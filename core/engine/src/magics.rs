//! Magic-bitboard attack lookups for the three sliding piece types.
//!
//! For a rook, bishop, or queen on square `s`, the squares it attacks depend
//! on which squares *between* `s` and the board edge are occupied. Naive
//! computation casts rays in each direction, checking one square at a time
//! — `O(N)` per query. The magic-bitboard trick turns this into `O(1)` at
//! the cost of a few-hundred-KB precomputed table:
//!
//! 1. For each square, compute the **relevant occupancy mask**: the set of
//!    squares whose occupancy actually changes the attack set. The piece's
//!    own square is excluded (it doesn't matter what's there, the piece
//!    blocks itself trivially) and so is each ray's terminal square on the
//!    board edge (the ray always either reaches that square or stops just
//!    before it, regardless of what's on it).
//! 2. Find a 64-bit integer `M` such that for every subset `O` of the mask,
//!    `((O * M) >> shift)` produces a unique index into a per-square
//!    attack table. `M` is discovered by trial with a seeded PRNG that
//!    emits sparse numbers (few bits set — empirically more likely to yield
//!    working magics).
//! 3. At query time: `attacks = table[offset + ((occupancy & mask) * magic >> shift)]`.
//!
//! Magic search runs lazily on first use, with a fixed PRNG seed so the
//! computed magics are deterministic across runs. The search takes a few
//! tens of milliseconds; subsequent lookups are a handful of cycles.
...
/// Seeds for the two magic searches. Any seed that converges quickly works;
/// these were picked by running the search and keeping ones with short times.
...
/// Run the magic search for every square of one slider type and return a
/// fully-populated `Slider`. Each square's per-square attack table is
/// appended to a shared `Vec<Bitboard>`, and each `Magic::offset` is set to
/// the square's slice's starting index.
