...
- **~~Port as an analytical-only detector:~~ DROPPED (2026-05-27):** Zugzwang. It needs a *search* (static eval can't see it — lichess runs depth-30), and more fundamentally it's a *state*, not an exploitable tactic — if every enemy move is bad you're already winning, so it prompts no student action. See the [Zugzwang section](#zugzwang-taggerzugzwangpy).
...
**Verdict (revised 2026-05-27): DROPPED — not a teaching tactic.** The predicate: for a pov move in the solution, if the position isn't check and has ≤ 15 legal moves, compare the eval of the real position against the eval after a *null move* (giving the opponent a free tempo); if the side-to-move's win-chance is ≥ 0.3 *lower* than after the null move, they're in zugzwang.

Two reasons it was dropped rather than built:

1. **It needs a search, not a static predicate.** Zugzwang is *invisible to static eval* — the pieces barely move under a null move, so the static score hardly changes; only a search reveals that *every* actual move worsens the position. That is why `zugzwang.py` runs a depth-30 engine search (`score_of`). It is also why the existing PVs don't reveal it: a negamax position's score *is* its best move's score (same number), so there is no internal baseline to compare against — the only baseline that isolates "I'm worse because I must move" is the *null-move* position, which is never one of the searched lines. Building it correctly means one extra null-move search per probed position (feasible on the analytical/retrospective path, which already owns an engine — cheaper than first framed — but still a search, not a static scan).
2. **More fundamentally, zugzwang isn't an *exploitable* tactic — it's a state.** If every enemy move is bad, you are already winning; there is no move you "take advantage of," and naming it prompts no student action (you can't choose to pass). It's "a thing that's already happening," unlike a fork/pin/overload that names *what to do*.

The predicate is preserved here for the record; if a future surface ever wants a "you're in zugzwang" annotation it is a one-null-move-search add to the analytical layer, but it is **not** on the roadmap.
...
- **W4-impl 3 — Sacrifice classification + `win_chances` utility. ✅ ENGINE SIDE LANDED (2026-05-27).** `win_chances` ported to [`analysis/win_chances.rs`](core/engine/src/analysis/win_chances.rs) — lila sigmoid (MULTIPLIER −0.00368208), mate→±1, **normalizing internal cp (PAWN_EG=213) → conventional pawn=100 first** (`v·100/213`, same convention as `traps::logic` and SF's UCI). Constant ported as-is; refit against our classical eval is a documented follow-up. Added `TacticHit.sacrifice: bool` + standalone `TacticPattern::Sacrifice`; `is_sacrifice` ports `cook.py:sacrifice` (down ≥2 points by the mover's 2nd move; opponent-promotion in the line disqualifies). The one-ply-guarantee misfire fix (memory `project_threat_signal_revisit`) is **scoped to the tactic layer** (user decision): in `compute_tactic_outcome`, a material-down line that is *sound* (`win_chances(user score) ≥ SOUND_SACRIFICE_WC = 0.0`) surfaces as a played `Sacrifice` (when no geometric pattern fires) and **suppresses the spurious `user_walked_into`**; a losing dump stays unsurfaced and its walked-into free-piece-capture still fires. The static `threats_outcome` filter is unchanged — it has no eval access, so a search-based hardening there is a separate, larger effort (deferred). The broader per-slot uniqueness/"don't nag" `win_chances` gating belongs to **W4-impl 4** (below). 790 engine tests, clippy clean.
- **W4-impl 4 — Second-wave patterns. ✅ ENGINE SIDE LANDED (2026-05-27).** Six multi-ply detectors (`TacticPattern::{Intermezzo, Deflection, Attraction, Interference, Clearance, XRay}`) in `tactic_outcome/detectors.rs`, ports of the matching `cook.py` predicates. A `line_boards` helper replays the `pv` (`boards[i]` = after `pv[0..i]`) to mirror lichess's `mainline`/`parent`/`grandpa` navigation; patterns resolve at the mover's 2nd move (`pv[2]`) or 3rd (`pv[4]`), bounded to a 5-ply window so a named tactic stays attributable to the user's move (intermezzo also uses `prior_move`). Appended to the priority chain after the single-move patterns. Fixtures ported from lichess `tagger/test.py` and tested **per-detector in isolation** (`detectors_tests.rs`) — the full single-hit chain returns the most immediate pattern while lichess assigns several tags, so isolation is the faithful test. **Plus the suppression/uniqueness gates**: `missed_tactic_worth_flagging` suppresses `user_missed_tactic` unless the best move beats the user's by a real win% gap (`MISS_MIN_WC_GAP=0.15`) and the user isn't already winning (`ALREADY_WINNING_WC=0.80`) — lichess's generator gates ported in spirit. 802 engine tests, clippy clean. Note: lichess's only `test_clearance` fixture is commented out and genuinely fails the predicate (check + prior king move) — replaced with a clean hand-built clearance.
- **W4-impl 5 — Mate-pattern library (engine-available, low UI priority). ✅ ENGINE SIDE LANDED (2026-05-27).** Seven terminal-node detectors in [`tactic_outcome/mate.rs`](core/engine/src/analysis/tactic_outcome/mate.rs) — ports of the `cook.py` mate sub-functions (back-rank, smothered, anastasia, hook, arabian, boden/double-bishop, dovetail), applied in `cook`'s exclusive order. `detect_mate_pattern` replays the full `pv`, gating on leaf-is-checkmate + mating-move-is-mover's (odd-length line; an opponent's mate is named instead via the `user_walked_into` slot) + `pv.len() ≤ MATE_MAX_PLIES=9`. **Data model (user-chosen, mirrors the Wave-3 `sacrifice` flag):** public `MatePattern` enum + `TacticHit.mate_pattern: Option<MatePattern>` annotated onto the geometric hit (orthogonal — a fork-into-back-rank-mate carries both, as lichess tags both), with a standalone `TacticPattern::Checkmate` synthesized when no geometric pattern fires. `MatePattern::surfaced_by_default()` = `true` only for back-rank/smothered (a *UI* distinction; all seven always detected). 815 engine tests, clippy clean. **Deferred UI** (per directive 3): the `Checkmate`/`MatePattern` retrospective cards. Back-rank fixtures ported from `tagger/test.py`; smothered/anastasia/arabian/boden/double-bishop hand-built as real mate-in-1 lines, hook/dovetail as direct sub-detector geometry checks (lichess ships no fixtures for these).
- **W4-impl 6 — Optional motifs + overloading. ✅ ENGINE SIDE LANDED (2026-05-27); zugzwang DROPPED.** `TacticPattern::AttackingF2F7` + `TacticPattern::UnderPromotion` ported into the chain (low priority — appended after the wave-4 patterns), and `is_checkmate` hoisted to `tactic_outcome/mod.rs` so the under-promotion detector and the mate detectors share it. **Overloading** built from scratch as a *pre-move scan* (lichess's `cook.py:overloading` is a `return False` stub — nothing to port): [`analysis/overloading.rs`](core/engine/src/analysis/overloading.rs) `find_overloaded(pos, victim) -> Vec<OverloadedPiece>`, strict predicate = an enemy piece that is the **sole defender of ≥ 2** of its own pieces the attacker is hitting. It is *not* a chain `TacticPattern` (user-chosen): overloading is a static "notice this" property, not a move you play, so it's its own analytical surface (a future coaching card / overlay, like trapped-piece) — the *exploitation* of an overload already surfaces as RemovingDefender/Deflection. **Zugzwang dropped by decision** (see the zugzwang section below). 822 engine tests, clippy clean.
