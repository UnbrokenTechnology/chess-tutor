...
Full design + roadmap: [`PLAN-cli.md`](PLAN-cli.md) (the command surface) and [`PLAN-tactic-escape.md`](PLAN-tactic-escape.md) (the escape-detection model). The named-pattern detector (`tactics`), the aggregator (`explain`), the move-critique command (`critique`), the latent-threat scanner (`--latent` / `danger:`), and escape detection have all landed.
