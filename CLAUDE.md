...
| `chess-tutor tactics [FEN]` | named-pattern detector for both sides + overloaded-defender scan. **A pattern the side-to-move can't actually cash** — one the opponent refutes with a *forcing* reply — is rendered `best tactic: NONE that win … REFUTED` with the refuting move named, so you never mistake a refuted pin/fork for your own weapon. The same geometry is often the *opponent's* resource (your "pin" is really their discovered attack) — cross-check the `danger:` block. `--latent` adds standing threats; `--check-followups` adds two-step forcing lines |
| `chess-tutor explain [FEN]` | one-shot aggregator: summary + threats + tactics (latent + check-followups) + a depth-N search. "Give me everything on this position" — the right first call when you're unsure which surface you need *and you have no specific move to judge* (for that, use `critique`) |
| `chess-tutor search [FEN]` | depth-N search → best move + PV + score. **Lean by default** — the per-term eval table and search stats are behind `--debug`; reach for `eval`/`explain` when you want terms. `--annotate` adds a tactic-pattern line; `--force-include <move>` scores a specific move against the best line |
| `chess-tutor critique <FEN-before> <move>` | **judging a move you already played? START HERE.** Scores the move against the engine's best line on the position *before* it and explains the swing — fires an `ALLOWED, NOT MISSED` reframe when the move handed over a winning/equal position, and says so plainly when the move was fine. It indexes on the **move**, which a bare `explain`/`threats` scan does not. Move accepts SAN (`Qc5+`) or UCI (`c4c5`) |
...
**Rule of thumb — judging a move you played? index on the *move*, not the position.** "Why was `X` bad?" is a question about `X`, not about the position in general. Run `chess-tutor critique <FEN-before> X` **first** — it scores `X` against the best line and tells you what the move *allowed*. The trap: answering from a bare `explain` / `threats` scan invites reporting a position-level feature (*"your queen is hanging"*) as "the key" — but that feature may describe the position *before* the move and not even be true after it. The single most important thing about a played move is **the swing it caused and what the opponent gets to do because of it**; `critique` puts that first. (Observed failure: an agent asked "why was Qc5+ bad?" led with "the queen on c4 is hanging" — true before the move, irrelevant after it, because it analysed the position instead of the move.)
...
**Rule of thumb — a pattern your opponent can refute is not your tactic.** When the side-to-move's best "tactic" has a *forcing* escape (a check, in-between capture, both-defending move, retreat, counter-threat), `tactics` renders it `best tactic: NONE that win … REFUTED` and names the refuting reply. The geometry is real and on the board — but you **cannot cash it**, so do not play as if you can. Crucially, the same pieces are often the *opponent's* standing resource against you: a pin you think you own (Re1→be5→qe6) is really their discovered attack (qe6→be5→Re1), sprung with tempo by a check (`…Bxh2+`) that denies you the move to punish it. By default a relative pin *wins* — to discover the attack the opponent must move the blocker, and then it's your turn to cash the pin first; it only **reverses** when that blocker-move is itself forcing. So always cross-check the `danger:` header (which names exactly this) before concluding you're ahead.

The named-pattern detector (`tactics`), the aggregator (`explain`), the move-critique command (`critique`), the latent-threat scanner (`--latent` / `danger:`), and escape detection have all landed — the command table above is the current surface. The design notes that drove this work (`PLAN-cli.md`, `PLAN.md`, `PLAN-tactic-escape.md`) have been retired to git history now that the features shipped; rationale lives in the relevant `analysis/` module `//!` docs (`latent_threats.rs`, `check_followups.rs`, `tactic_escape.rs`).
