...
Tests: **815 engine (+4 ignored) + 105 narration + 33 cli + 27 ui = 980 passing**, clippy clean (bar a pre-existing `cloned_ref_to_slice_refs` lint from a clippy version bump in `threats_outcome/tests.rs` + `chess-tutor-ui`, untouched here).
...
815 engine tests pass, clippy clean (mod a pre-existing version-bump lint, above). **NEXT is W4-impl wave 6 (optional / deferred: attackingF2F7 motif; overloading-from-scratch — lichess stubbed it, `return False`; under-promotion; analytical-only zugzwang, build last). All UI surfacing — `RetrospectiveCategory::Tactic`, the `Checkmate`/`MatePattern` mate cards, the `TrappedPieces` overlay desktop wiring, coaching-panel names — stays deferred until the engine waves land.**
