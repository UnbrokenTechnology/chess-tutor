...
777 engine tests pass, clippy clean. **NEXT is W4-impl wave 3 (Sacrifice flag + `win_chances`, fixing the one-ply-guarantee misfire); all UI surfacing — `RetrospectiveCategory::Tactic`, the `TrappedPieces` overlay desktop wiring — stays deferred until the engine waves land.**
